function createV4SelectableForceDirectedGraph(svg, graph) {
    ...
    for (i = 0; i < graph.nodes.length; i++) {
        nodes[graph.nodes[i].id] = graph.nodes[i]["id"];
        nodes[graph.nodes[i].name] = graph.nodes[i]["name"];
    ...
    function searchNode(){
        var name = document.getElementById('searchTextField').value;
        /*var name = null;
        d3.select("#searchTextField").property("value", name);*/
        ...
        if( name !== null) {
            graph.nodes.forEach(function (d) {
                ...
                if (d.name === name) {
                    ...
                    d3v4.select('circle[id="58"]').dispatch('click');
                    return 0;
                }
                else
                    return -1;
    ...
    node.on("click", fade(.1)).on("dblclick", fade(1));
